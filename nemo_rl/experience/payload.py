...
def record_to_train_batch(
    ...
    *,
    ...
    Returns:
        ...
        token_mask, the loss-multiplier sample_mask, the raw mask_sample and
        truncated flags, total_reward, episode_success, violation counts, and
        optional routed experts, message-violation masks, and any packed or
        per-token multimodal model inputs carried by the completions.
    """
    ...
    # Must precede the prompt flatten below: it reuses the same message dicts.
    # Doing it only inside add_grpo_token_loss_masks_and_generation_logprobs
    # would be too late.
