...
"""The Apertus chat template emits BOS itself, so re-tokenizing must not add another.

Rendered prompts start with ``<s>`` (id 1). Tokenizing that text with the
``add_special_tokens=True`` default yields ``[1, 1, ...]`` -- a second BOS the
model never saw in training. It corrupts silently: no error, no shape change,
just a prompt that is off-distribution by one token. Every tokenizer() call on
an Apertus path must pass add_special_tokens=False after templating.
"""
...
APERTUS_TOKENIZER_PATH = os.environ.get(
    "APERTUS_TOKENIZER",
    "/capstor/store/cscs/swissai/infra01/MLLM/tokenizer/Apertus-v1.5-8B-official",
)

pytestmark = pytest.mark.skipif(
    not os.path.isdir(APERTUS_TOKENIZER_PATH),
    reason=f"Apertus tokenizer not staged at {APERTUS_TOKENIZER_PATH}",
)
...
def tokenizer():
    ...
    return AutoTokenizer.from_pretrained(APERTUS_TOKENIZER_PATH)
