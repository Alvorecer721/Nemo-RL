...
def _fresh_path_state(monkeypatch):
    ...
    xfer._DEVICE_API_SUPPORT_BY_COMMUNICATOR.clear()
    yield
    xfer._DEVICE_API_SUPPORT_BY_COMMUNICATOR.clear()
...
def test_capability_is_queried_once_per_communicator(monkeypatch):
    process_group = _ProcessGroup(True)
    _xferdtensor_with_stubs(monkeypatch, process_group)
    _xferdtensor_with_stubs(monkeypatch, process_group)
    assert process_group.nccl_communicator.queries == 1
