...
@pytest.mark.parametrize(
    "name", ["model.embed_tokens.weight", "lm_head.weight", "model.language_model.embed_tokens.weight"]
)
...
def test_get_placements_shards_o_proj_rows_and_vocab_columns():
    dm = {"tp": 0}
    assert _shard_dim_at(get_placements("a.self_attn.o_proj.weight", dm, 2), dm, "tp") == 1
    assert _shard_dim_at(get_placements("model.embed_tokens.weight", dm, 2), dm, "tp") == 0
    assert _shard_dim_at(get_placements("lm_head.weight", dm, 2), dm, "tp") == 0


@pytest.mark.parametrize(
    "vocab,train_tp,train_multiple,gen_tp,expected",
    [
        (266752, 2, 128, 4, True),  # Apertus 70B: no padding on either side
        (154880, 8, 128, 8, False),  # GLM-5.1 vocab at TP8: 154880 % 1024 != 0 -> Megatron pads
        (128256, 2, 128, 8, False),  # 128256 / 8 = 16032 is not a multiple of 64 -> vLLM pads
        (128256, 2, 512, 4, False),  # 128256 % 1024 != 0 -> Megatron pads its shards
        (32000, 1, 128, 1, True),
    ],
)
def test_vocab_parallel_bulk_ok_requires_unpadded_shards_on_both_sides(
    ...
    assert (
        vocab_parallel_bulk_ok(
            ...
            make_vocab_size_divisible_by=train_multiple,
            gen_tp_size=gen_tp,
        )
        is expected
    )
