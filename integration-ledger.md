...
The historical TE 2.17.1 control (job 3301954, source `b7a6a2a61`) and the TE 2.18 MB1 control (3304520) have effectively identical mean full-step time over warm updates 2–4: **49.869 s versus 49.884 s**. Mean trainer time is **40.236 s versus 40.866 s** (+1.6%). Both use corrected Apertus 70B, GSM8K, 2k total context, global batch 768, MB1 and the same 16-node topology. Different asynchronous batches/node sets and the combined dependency/chunking changes prevent attribution to TE alone. Only three warm updates are available in the new control; this is not evidence of an end-to-end speedup. Exact series are in `microbatch-ab/te217_vs_te218.json` under the session records below.

The fresh local review after opening PR #29 found a boxed-answer scorer bug: numeric contents inside `text`, `textbf`, `mathrm` or `mathbf` were deleted with unit text. The correction preserves numeric contents and retains unit-suffix handling. All 16 new examples failed before correction; the full 32-case reward/environment suite passed in the qualified image (3304609, `0:0`, 1:29). A CI fetch correction avoids recursively requesting unavailable historical nested pins while fetching the target tree; current recursive checkout and submodule ancestry validation remain enabled. These source/CI corrections leave dependency fingerprints unchanged.
