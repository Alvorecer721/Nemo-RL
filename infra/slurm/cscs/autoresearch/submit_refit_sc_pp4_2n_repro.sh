...
RUN_ROOT=${REFIT_SC_RUN_ROOT:-/iopsstor/scratch/cscs/${USER:-$(id -un)}/nemo_rl_refit_sc_pp4_2n/$EXPECTED_HEAD}
SBATCH_LOG_ROOT=${REFIT_SC_LOG_ROOT:-$REPO_DIR/.tmp/slurm-logs/refit-sc-pp4-2n/$EXPECTED_HEAD}
RESERVATION=${REFIT_SC_RESERVATION-SD-69241-apertus-1-5-0}
...
export REFIT_SC_REPO_DIR=$REPO_DIR
export REFIT_SC_EXPECTED_HEAD=$EXPECTED_HEAD
export REFIT_SC_RUN_ROOT=$RUN_ROOT
...
exec "$SBATCH_BIN" \
  ...
  --exclusive \
  --time=01:05:00 \
  --job-name=refit-sc-pp4-2n \
  --output="$SBATCH_LOG_ROOT/slurm_%j.out" \
  --error="$SBATCH_LOG_ROOT/slurm_%j.err" \
  --export=ALL \
  "$REPO_DIR/infra/slurm/cscs/autoresearch/run_refit_sc_pp4_2n_repro.sh"
