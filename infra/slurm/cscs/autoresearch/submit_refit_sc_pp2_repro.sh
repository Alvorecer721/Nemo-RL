...
RUN_ROOT=${REFIT_SC_RUN_ROOT:-/iopsstor/scratch/cscs/${USER:-$(id -un)}/nemo_rl_refit_sc_pp2/$EXPECTED_HEAD}
SBATCH_LOG_ROOT=${REFIT_SC_LOG_ROOT:-$REPO_DIR/.tmp/slurm-logs/refit-sc-pp2/$EXPECTED_HEAD}
...
exec "$SBATCH_BIN" \
  ...
  --exclusive \
  --time=01:05:00 \
  --job-name=refit-sc-pp2 \
  --output="$SBATCH_LOG_ROOT/slurm_%j.out" \
  --error="$SBATCH_LOG_ROOT/slurm_%j.err" \
  --export=ALL \
  "$REPO_DIR/infra/slurm/cscs/autoresearch/run_refit_sc_pp2_repro.sh"
