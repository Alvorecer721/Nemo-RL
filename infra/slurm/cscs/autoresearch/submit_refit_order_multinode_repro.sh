...
RUN_ROOT=${REFIT_REPRO_RUN_ROOT:-/iopsstor/scratch/cscs/${USER:-$(id -un)}/nemo_rl_refit_order_multinode/$EXPECTED_HEAD}
SBATCH_LOG_ROOT=${REFIT_REPRO_LOG_ROOT:-$REPO_DIR/.tmp/slurm-logs/refit-order-multinode/$EXPECTED_HEAD}
...
exec "$SBATCH_BIN" \
  ...
  --exclusive \
  --time=00:45:00 \
  --job-name=refit-order-2n \
  --output="$SBATCH_LOG_ROOT/slurm_%j.out" \
  --error="$SBATCH_LOG_ROOT/slurm_%j.err" \
  --export=ALL \
  "$REPO_DIR/infra/slurm/cscs/autoresearch/run_refit_order_multinode_repro.sh"
