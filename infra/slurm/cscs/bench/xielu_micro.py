...
for label, shape in [
    ("decode  [16,1,21504]", (16, 1, 21504)),
    ("prefill [4096,1,21504]", (4096, 1, 21504)),
]:
    ...
    gb = x.numel() * 2 * 2 / 1e9
    print(
        f"{label}: eager-python {r_eager:8.1f} us | compiled {r_comp:7.1f} us ({gb / (r_comp / 1e6):5.0f} GB/s) | cuda-kernel {r_kern:7.1f} us ({gb / (r_kern / 1e6):5.0f} GB/s)"
    )
...
d = (
    (
        xielu_python(x, ap, an, beta_t, eps_t)
        - kernel.forward(x, ap, an, 0.5, -1e-6, False)
    )
    .abs()
    .max()
)
