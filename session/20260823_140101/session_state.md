...
- The frozen 88-node `windowed` baseline job `3182849` completed all ten SingleController training steps from source SHA `6366a7599`, including `SC run complete`. Its final step had loss `0.03883`, grad norm `0.3133`, reward `0.3281`, and generation KL `0.0004539`. The Python driver then returned exit code 2 during resource teardown, before the harness could run its trace and metric postchecks; the terminal artifact therefore records `failure_phase=training` even though training completed. The precise exit-2 source inside teardown remains to be isolated.
- Slurm consequently marked the original `afterok:3182849` dependency for ready-first job `3184823` as `DependencyNeverSatisfied`. The dependency was changed in place to `afterany:3182849`; job `3184823` is now running on 88 nodes under the existing authorization and reservation.
- Job `3184823` completed three correctness-green ready-first steps, then was explicitly cancelled by the user at 16:43 CEST because the two-replica rollout fleet was too slow. Times were `2456.12s`, `852.25s`, and `1559.06s`; all three had finite loss/grad/reward, generation KL below `0.000521`, no masked sequences, and no drops, stale evictions, or aborts. Preserve this as partial evidence rather than claiming a ten-step sampler verdict.
- The topology is strongly rollout-bound: 288 training GPUs spend only about `67s` in the optimizer step while 64 inference GPUs in two TP32 replicas require `741-1447s` of exposed generation on steady steps. The corrected first-three-step training-worker idle ratio is about `90.4%`. Upstream `nemo_rl/algorithms/utils.py` currently reports zero for meaningful waits because its `exposed_generation_time > 0.1` conditional is inverted; this is an instrumentation bug, not evidence that the trainer is busy.
- Version semantics are source- and log-verified: step 1 trains policy `v0` and creates `v1`; step 2 trains policy `v1` on 16 eligible `v0`/`v1` groups and creates `v2`. Its 21 remaining `v0` in-flight groups were aborted only after the step-2 update made them older than the one-version window. They consumed rollout slots and delayed fresh work indirectly; the trainer did not wait for those specific groups to finish.
...
- `ready_first` is a committed recipe setting at `async_rl.sampler.name`, not an OS-controlled behavior. `GLM_DEPENDENCY` only sequences the Slurm jobs and `GLM_RUN_DIR` only supplies the output path; a launcher environment may select a recipe file, but the sampler value itself remains in YAML.
...
- After the matched run, add the planned root `FORK_PATCHES.md` before the next upstream sync. It must inventory NeMo-RL, Bridge, and nested Megatron-LM patches with upstream base, fork commit, rationale, validation, upstream PR/status, and the condition under which each patch can be dropped.
- Next experiment: keep ready-first and all correctness settings fixed, increase only generation resources from 16 to 32 nodes, and increase the cluster allocation from 88 to 104 nodes. This yields four TP32 vLLM replicas against the unchanged 72-node Megatron trainer.
