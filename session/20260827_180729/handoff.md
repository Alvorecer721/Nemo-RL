...
Job `3219237` proved that vLLM 0.25.1's `DeepSeekMTP` does not support `use_local_argmax_reduction`; it failed before loading weights. The corrected recipe disables only that optional optimization, and the preflight now rejects reintroducing it. Re-run focused tests, commit, and resubmit `infra/slurm/cscs/autoresearch/submit_glm51_sc_mtp3.sh`. Freeze the checkout while it runs. Acceptance requires the MTP disk-load marker, nonzero and internally consistent speculative acceptance counters, all existing ten-step learning/KL/tail/Router Replay/CP-identity gates, and scheduler exit `0:0`. Compare warm-step total/exposed-generation time and valid-token throughput directly with `3217663`; startup time is not a throughput result.
