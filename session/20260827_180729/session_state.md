...
- First MTP3 attempt: job `3219237` failed before weight loading because vLLM 0.25.1's `DeepSeekMTP` lacks `get_top_tokens()` and rejects `use_local_argmax_reduction`. This is an unsupported optional optimization, not a model, topology, or cluster failure; the corrected challenger keeps native MTP and disables only local argmax.
