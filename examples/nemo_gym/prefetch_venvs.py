...
def prefetch_nemo_gym_venvs(config_paths: list[str]) -> None:
    ...
    # Gym setup scripts use ``uv pip install --no-cache``, which is
    # incompatible with the image-wide symlink mode. Preserve upstream's
    # actor-specific hardlink override while using the centralized builder.
