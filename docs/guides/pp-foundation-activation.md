...
Both settings are required for the measured memory benefit. On GH200, with batch 1, sequence length 2048, vocabulary 266752 and TP2, logprob/gradient parity passed. Peak incremental allocation fell from 3.278 GB to 1.367 GB; the isolated forward/backward operation increased from 6.456 ms to 13.697 ms. Multiplying the additional 7.240 ms by 128 microbatches gives an estimated 0.927 seconds per optimizer update. This is an operation-count estimate, not a matched full-step A/B measurement.

The 58% reduction is specific to that temporary allocation, concentrated on the pipeline stage computing vocabulary logprobs. It does not establish a 58% reduction in whole-worker memory or extra KV-cache capacity on the separate rollout nodes. The first performance candidate is training microbatch 2 with global batch 768 unchanged: at DP6, the microbatch count falls from 128 to 64. Larger model activations and other pipeline stages can still limit whether it fits. Separately test reduced selective recomputation, confirming the effective model configuration, then consider longer sequences for the 12k training objective. Longer sequences increase work. Compare full-update time and actual peak worker memory, preserving the data, loss and optimizer settings; check numerical behavior within tolerance.
...
The GPU runtime reports NCCL **2.28.9**. Build headers and installed wheel metadata are not substitutes for that runtime measurement. `NVTE_WITH_NCCL_EP=0` remains set; the newer M2N NCCL transport is not qualified by this image.
