...
- **Re-enabling** (should a future measured regime justify it): re-add
  `+policy.generation.vllm_cfg.env_vars.PYTHONPATH=$XIELU_SITE` to the launcher, purge
  compile caches, and re-run the paired same-node A/B before believing any number.
